...
describe('Starting tests for blockchain_proj', function() {
  ...
  it('website title should be blockchain_proj', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('blockchain_proj');
    })
  });

  it('network-name should be blockchain_proj@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('blockchain_proj@0.0.1.bna');
    });
  });

  it('navbar-brand should be blockchain_proj',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('blockchain_proj');
    });
  });
